...
def binarize_node_mask(graph: tv.GraphDict, 
                       mask: np.ndarray,
                       threshold_0: float = 0.5,
                       threshold_1: float = 1.0,
                       ) -> np.ndarray:
    node_adjacency = graph_node_adjacency(graph)
    weights = node_adjacency * mask[:, np.newaxis]
    weights_broadcast = np.sum(weights, axis=0)
    
    mask_binary = np.logical_or(
        (mask > threshold_0),
        (weights_broadcast > threshold_1),
    ).astype(float)
    
    return mask_binary


def graph_node_mask(graph: tv.GraphDict,
                    value: float = 1.0,
                    ) -> np.ndarray:
    num_nodes = len(graph['node_indices'])
    mask = np.ones(shape=(num_nodes, ) ) * value
    return mask


def graph_edge_mask(graph: tv.GraphDict,
                    value: float = 1.0,
                    ) -> np.ndarray:
    num_edges = len(graph['edge_indices'])
    mask = np.ones(shape=(num_edges, ) ) * value
    return mask
